...
logger.level("Missing", no=21, color="<yellow>")
...
def md5sum(filepath: Path, pbar=None, blocksize: int = 65536) -> str:
    ...
    try:
        ...
        logger.critical(f"The hell: {filepath}")
...
class HumbleApi:
    ...
    def get_product_list(self):
        with tqdm(
            ...
            #             self.orders_num), self.order_key_list
            #     ):
            #         if order:
    ...
    def download(self, item, pbar):
        ...
        if self.check_file(item, filename, pbar):
